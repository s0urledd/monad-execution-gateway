...
Port `8443` on localhost, auto-restart, event ring mounted read-only. Custom port: edit `docker-compose.yml` command to `--server-addr 127.0.0.1:9090`.
...
|----------|---------|-------------|
| `--server-addr` | `127.0.0.1:8443` | Listen address (localhost only) |
